...
qDebug() << "set the camera in rgb24  " << errCode;	
		...
		while(!failed && !GCamera.Abort)
		{
			...
			else
			{
				...
				//	qDebug() << "Frames: " << GCamera.Frames[Index];
					qDebug() << "Index: " << Index;
				}
...
private slots:
	...
	//	 unsigned long line_padding = 0;
     //    unsigned long line_size = 0;
    //     unsigned long buffer_size = 0;
    //     unsigned char* buffer_rgb = new unsigned char[4177920];;

	   //  unsigned char* buffer_rgb  = new unsigned char[2];;
...
//      line_padding = ULONG_PADDING((GCamera.Frames[Index].Width)*3);
//      line_size = ((GCamera.Frames[Index].Width)*3) + line_padding;
//      buffer_size = line_size * GCamera.Frames[Index].Height;
     
  //    buffer_rgb = new unsigned char[buffer_size];
    //   buffer_rgb     = new unsigned char[buffer_size];

//	 PvUtilityColorInterpolate(&GCamera.Frames[Index], &buffer_rgb[2], &buffer_rgb[1],&buffer_rgb[0],0, line_padding);

//buffer_rgb[2]  =  (buffer_rgb[1] + buffer_rgb[0])/3;
...
//       qDebug() <<"buffer_rgb[2] : "<< &buffer_rgb[2];
//       qDebug() <<"buffer_rgb : "<< buffer_rgb;

  //     (unsigned char*)GCamera.Frames[Index].ImageBuffer = buffer_rgb[2];

qDebug() << "Frame Width :"<<GCamera.Frames[Index].Width << "Frame Height :"<<GCamera.Frames[Index].Height << "Format :"<<GCamera.Frames[Index].Format << "BayerPattern :"<<GCamera.Frames[Index].BayerPattern;


qDebug() <<"FrmCmp : "<<FrameCompleted << " FrmDrp :" <<FrameDropped << " PckCmp :" <<PacketReceived << " PckMss :" <<PacketMissed << " FrmRt :" <<FrameRate << " imageSize :" <<imageSize;
//qDebug() <<"ImgBuf : "<< *(unsigned char*)GCamera.Frames[Index].ImageBuffer;
//qDebug() <<"ImgBuf : "<< GCamera.Frames[Index].ImageBuffer;
...
// my_edit
  ...
  //      QImage image(buffer_rgb, 1360,1024,1360,QImage::Format_Indexed8);
