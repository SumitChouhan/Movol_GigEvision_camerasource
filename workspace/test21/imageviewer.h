...
char* pBufferforbandrange;
unsigned long pBufferforbandrangesize;
unsigned long* pSizeforrange;
...
PvAttrUint32Set(GCamera.Handle,"Height",800);

PvAttrUint32Set(GCamera.Handle,"Width",950);
...
qDebug() << "[RSH][" <<__FILE__<< ":" <<__LINE__<< ":" <<__FUNCTION__<<"()] : " << failed;
  ...
  // Take_Note					frame->imageData = (char*) Camera.Frame[Index].ImageBuffer;
...
/*
if(!(Err = PvAttrList(GCamera.Handle,&Attrs,&Count)))
...
if (!(Err = PvAttrRangeEnum(GCamera.Handle,"BandwidthCtrlMode",pBufferforbandrange,8,pSizeforrange)))

{

qDebug() << pBufferforbandrange << pSizeforrange;

}
else 
qDebug() << "Error in getting bandwidth contro mode range";
...
*/
		...
		QImage image((unsigned char*)GCamera.Frames[Index].ImageBuffer,950,800,950*3,QImage::Format_RGB888);
