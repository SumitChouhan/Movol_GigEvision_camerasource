...
int client; 
unsigned int len = sizeof(remoto);
int slen;
//char buffer[4096];
char socket_buffer[250000];

//qDebug() << "Socket Variable initialized";
...
qDebug() << "problem in setting pixel format in camera ";
...
PvAttrUint32Set(GCamera.Handle,"Height",500);

PvAttrUint32Set(GCamera.Handle,"Width",500);
...
local.sin_addr.s_addr		=  inet_addr("192.168.0.80");
...
qDebug() << "bind succesfull" << inet_ntoa(local.sin_addr) << local.sin_port << local.sin_family; 
...
qDebug() << "went throw the socket worker line codes" <<__FILE__<< ":" <<__LINE__<< ":" <<__FUNCTION__<<"()] : " << failed;
...
qDebug() <<  "the client which is trying to connect"; //<< remoto.sin_addr ;


strcpy(socket_buffer,"Welcome!\n\0");
//strcpy(socket_buffer,( char*)GCamera.Frames[Index].ImageBuffer);


if( send(client,socket_buffer, strlen(socket_buffer),0)) { 

qDebug() <<  "Data send to client\n";

//while(1){

if( (slen = recv(client,socket_buffer,LEN,0)) > 0 )

{

socket_buffer[slen-1] = '\0';
...
qDebug() << "data not received by client %s \n" << socket_buffer;
//}
}
...
qDebug() << "Waiting for frame to return to host...";
  ...
  qDebug() << "[RSH][" <<__FILE__<< ":" <<__LINE__<< ":" <<__FUNCTION__<<"()] : " << failed;
		...
		{
			qDebug() << "failed!!!";
qDebug() << "ImageBuffer " << GCamera.Frames[Index].ImageBuffer;
qDebug() << "ImageBufferSize " << GCamera.Frames[Index].ImageBufferSize;	
qDebug() << "AncillaryBuffer " << GCamera.Frames[Index].AncillaryBuffer;
qDebug() << "AncillaryBufferSize " << GCamera.Frames[Index].AncillaryBufferSize;
qDebug() << "Context[4] " << GCamera.Frames[Index].Context[4];
qDebug() << "_reserved1[8] " << GCamera.Frames[Index]._reserved1[8];
qDebug() << "Status" << GCamera.Frames[Index].Status;
qDebug() << "ImageSize " << GCamera.Frames[Index].ImageSize;
qDebug() << "AncillarySize" << GCamera.Frames[Index].AncillarySize;
qDebug() << "Width " << GCamera.Frames[Index].Width;
qDebug() << "Height " << GCamera.Frames[Index].Height;
qDebug() << "RegionX " << GCamera.Frames[Index].RegionX;
qDebug() << "RegionY " << GCamera.Frames[Index].RegionY;
qDebug() << "Format" << GCamera.Frames[Index].Format;
qDebug() << "BitDepth " << GCamera.Frames[Index].BitDepth;
qDebug() << "BayerPattern " << GCamera.Frames[Index].BayerPattern;
qDebug() << "FrameCount" << GCamera.Frames[Index].FrameCount;
qDebug() << "TimestampLo " << GCamera.Frames[Index].TimestampLo;
qDebug() << "TimestampHi " << GCamera.Frames[Index].TimestampHi;
qDebug() << "   _reserved2[32]; " << GCamera.Frames[Index]._reserved2[32];
		}
...
// Socket_programming //
		...
		//QImage image((unsigned char*)GCamera.Frames[Index].ImageBuffer,1360,1024,1360*3,QImage::Format_RGB888);
		QImage image((unsigned char*)GCamera.Frames[Index].ImageBuffer,500,500,500*3,QImage::Format_RGB888);
