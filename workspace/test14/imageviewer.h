...
public:
	...
	bool CameraStart()
	{
		...
		if((errCode = PvCaptureAdjustPacketSize(GCamera.Handle,1500)) != ePvErrSuccess)
		{
...
// my_edit
		...
		if((PvAttrFloat32Set(GCamera.Handle,"FrameRate",1) != ePvErrSuccess) ||(PvAttrEnumSet(GCamera.Handle,"FrameStartTriggerMode","FixedRate") != ePvErrSuccess) ||
...
qDebug() << "_reserved2[32]; " << GCamera.Frames[Index]._reserved2[32];
	...
	QImage image((unsigned char*)GCamera.Frames[Index].ImageBuffer, 1360,1024,1360,QImage::Format_Indexed8);

    //   image.loadFromData(const uchar *buf, int len, const char *format = 0);
   //    image.loadFromData((unsigned char*)GCamera.Frames[Index].ImageBuffer,QImage::Format_Indexed8);	
   ...
   //     image = image.convertToFormat(QImage::Format_RGB888,Qt::ColorOnly);
