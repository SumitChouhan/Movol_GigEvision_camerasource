...
public:
	...
	bool CameraStart()
	{
		...
		for(int i=0;i<FRAMESCOUNT && !failed; i++)
		{           
			if((errCode = PvCaptureQueueFrame(GCamera.Handle,&(GCamera.Frames[i]),doWork)) != ePvErrSuccess)
			{
...
// my_edit  
		    ...
		    { 
				//likely camera unplugged
				qDebug() << "succes Pvwaitforcameradone " << errCode;
...
// my_edit
		...
		if((PvAttrFloat32Set(GCamera.Handle,"FrameRate",2) != ePvErrSuccess) ||
			(PvAttrEnumSet(GCamera.Handle,"FrameStartTriggerMode","Freerun") != ePvErrSuccess) ||
			(PvAttrEnumSet(GCamera.Handle,"AcquisitionMode","Continuous") != ePvErrSuccess))
// ||
	//		(PvCommandRun(GCamera.Handle,"AcquisitionStart") != ePvErrSuccess))
		{		
...
*/
	...
	//	QImage image((unsigned char*)GCamera.Frames[Index].ImageBuffer, 1360,1024,1360,QImage::Format_Indexed8);
		...
		image.fromData((unsigned char*)GCamera.Frames[Index].ImageBuffer,QImage::Format_RGB888);
