...
//PvAttrUint32Set(GCamera.Handle,"Height",480);

//PvAttrUint32Set(GCamera.Handle,"Width",640);
...
qDebug() << "[RSH][" <<__FILE__<< ":" <<__LINE__<< ":" <<__FUNCTION__<<"()] : " << failed;
		...
		while(!failed && !GCamera.Abort)
		{
			...
			while((errCode = PvCaptureWaitForFrameDone(GCamera.Handle,&GCamera.Frames[Index],10000)) == ePvErrTimeout)
			{
  qDebug() << "[RSH][" <<__FILE__<< ":" <<__LINE__<< ":" <<__FUNCTION__<<"()] : " << failed;
...
qDebug() << "   _reserved2[32]; " << GCamera.Frames[Index]._reserved2[32];
		...
		QImage image((unsigned char*)GCamera.Frames[Index].ImageBuffer,640,480,640*3,QImage::Format_RGB888);
